...
if (count($_POST) > 0) {
    ...
    if ($_POST['type'] == 1) {
        ...
        $sql = "INSERT INTO `ttypeproduits`(`nomTypeProduit`,alerteTypeProduit) VALUES ('$nomTypeProduit',1)";
...
if (count($_POST) > 0) {
    ...
    if ($_POST['type'] == 2) {
        ...
        $nomTypeProduit = $_POST['nomTypeProduit'];
        $alerteTypeProduit = $_POST['alerte'];

        // Encodage des guillemets / apostrophes
        $nomTypeProduit = addslashes($nomTypeProduit);

        $sql = "UPDATE `ttypeproduits` SET `nomTypeProduit` = '$nomTypeProduit', `alerteTypeProduit` = $alerteTypeProduit WHERE `ttypeproduits`.`ttypeproduitsPK` = $ttypeproduitsPK;";
